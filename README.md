...
- **Bybit API + WebSockets + JSON** - market-data for trading pairs in <ins>MGw</ins>.
- **Kafka** for transmitting Levels10 - an order book storing price levels[10] and volumes.
...
- ~~add **Protobuf**' <ins>proto</ins> & **gRPC** server to <ins>MGw</ins>~~
- register <ins>MGw</ins> in **Consul**
- add **Loki** logs to services (50%)
- set up **Grafana + Loki + Prometheus** server for logs and metrics
- **Avalonia** Client App <ins>MPr</ins> to receive market-data from <ins>MGw</ins> and show it to user
